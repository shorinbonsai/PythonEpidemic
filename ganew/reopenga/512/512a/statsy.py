...
def main():
    ...
    #     p0 = 26
    locky = "512a_random50graph.dat"
    with open(locky + "_stats2.txt", mode='w')  as f:
        print("Exp\t" + "Best\t" + "Mean\t" + "StdDev", file=f )
        # for i in test:
        
        lockdown_graph = make_adj_lists(locky)
        ...
        for j in range(30):
            ...
            while score <5:
                ...
                result.append(score)
        print(locky + "\t" + str(min(result)) + "\t" + str(mean(result)) + "\t" + str(np.std(result)), file=f)

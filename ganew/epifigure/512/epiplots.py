...
def main():
    ...
    strict50 = [np.array(x) for x in stuff50[0]]
    strict50 = [np.mean(k) for k in zip(*strict50)]
    medium50 = [np.array(x) for x in stuff50[1]]
    medium50 = [np.mean(k) for k in zip(*medium50)]
    lax50 = [np.array(x) for x in stuff50[2]]
    lax50 = [np.mean(k) for k in zip(*lax50)]
    skeptical50 = [np.array(x) for x in stuff50[3]]
    skeptical50 = [np.mean(k) for k in zip(*skeptical50)]


    lengths = [len(strict50), len(medium50), len(lax50), len(skeptical50)]
    data50 = [strict50, medium50, lax50, skeptical50]
    max_length = max(lengths)
    x = [n for n in range(max_length)]
    for el in data50:
        s_len = len(el)
        for _ in range(max_length - s_len):
            el.append(0)
    fig = matplotlib.pyplot.figure()
    fig.set_dpi(400)
    fig.set_figheight(4)
    plot = fig.add_subplot(111)
    plot.plot(x, strict50, label="Strict")
    plot.plot(x, medium50, label="Medium")
    plot.plot(x, lax50, label="Lax")
    plot.plot(x, skeptical50, label="Skeptical")
    plot.set_xticks(x)
    plot.legend()
    fig.tight_layout()
    fig.savefig(outputfile + "rem50epi.png")


    strict70 = [np.array(x) for x in stuff70[0]]
    strict70 = [np.mean(k) for k in zip(*strict70)]
    medium70 = [np.array(x) for x in stuff70[1]]
    medium70 = [np.mean(k) for k in zip(*medium70)]
    lax70 = [np.array(x) for x in stuff70[2]]
    lax70 = [np.mean(k) for k in zip(*lax70)]
    skeptical70 = [np.array(x) for x in stuff70[3]]
    skeptical70 = [np.mean(k) for k in zip(*skeptical70)]

    lengths = [len(strict70), len(medium70), len(lax70), len(skeptical70)]
    data70 = [strict70, medium70, lax70, skeptical70]
    max_length = max(lengths)
    x = [n for n in range(max_length)]
    for el in data70:
        s_len = len(el)
    ...
    fig.set_figheight(4)
    plot = fig.add_subplot(111)
    plot.plot(x, strict70, label="Strict")
    plot.plot(x, medium70, label="Medium")
    plot.plot(x, lax70, label="Lax")
    plot.plot(x, skeptical70, label="Skeptical")
    plot.set_xticks(x)
    plot.legend()
    fig.tight_layout()
    fig.savefig(outputfile + "rem70epi.png")

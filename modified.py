...
class MainWindow:
    ...
    def __init__(self, nodes: int, edges: int, length: int, adj_lists: [], p0: int = 0):
        ...
        self.img_label = tk.Label(
            frame, image=photo, bg='green', anchor="center")
        self.img_label.image = photo
        self.img_label.grid(row=0, columnspan=2, padx=2, pady=2)

        prev_btn = tk.Button(frame, text="Previous Day",
                             anchor="center", command=self.prev_img)
        prev_btn.grid(row=1, column=0, padx=2, pady=2, sticky="E")

        prev_btn = tk.Button(frame, text="Next Day",
                             anchor="center", command=self.next_img)
        ...
        gen_btn = tk.Button(frame, text="Generate New Random Network",
                            anchor="center", command=self.gen_network)
        gen_btn.grid(row=5, column=1, padx=2, pady=2, sticky="W")

        PLC_btn = tk.Button(frame, text="Generate New PLC Network",
                            anchor="center", command=self.gen_PLC)
        PLC_btn.grid(row=6, column=0, padx=2, pady=2, sticky="E")

        ER_btn = tk.Button(frame, text="Generate New ER Network",
                           anchor="center", command=self.gen_ER)
        ER_btn.grid(row=6, column=1, padx=2, pady=2, sticky="W")

        WS_btn = tk.Button(frame, text="Generate New WS Network",
                           anchor="center", command=self.gen_WS)
        WS_btn.grid(row=7, column=0, padx=2, pady=2, sticky="E")

        sim_btn = tk.Button(frame, text="Simulate 50 Epidemics",
                            anchor="center", command=self.simulate_epis)
        sim_btn.grid(row=7, column=1, padx=2, pady=2, sticky="W")

        ttk.Separator(frame, orient='horizontal').grid(
            row=8, columnspan=2, sticky="EW")
    ...
    def gen_network(self):
        if self.txt_check():
            ...
            self.adj_lists, nodes = make_network(
                "edgelists.txt", self.nodes, self.edges)
            edg_list = get_edge_list("edgelists.txt")
            # epi_log = run_epi(self.adj_lists, self.nodes, self.p0)
            epi_log = run_epi_percent(self.adj_lists, self.nodes, self.p0)
    ...
    def gen_PLC(self):
        if self.txt_check():
            ...
            self.edges = int(self.edge_txt.get("1.0", 'end-1c'))
            self.p0 = int(self.p0_txt.get("1.0", 'end-1c'))
            self.adj_lists, nodes = powerlaw_cluster(
                "edgelists.txt", self.nodes, self.edges, 0.2)
            edg_list = get_edge_list("edgelists.txt")
            # epi_log = run_epi(self.adj_lists, self.nodes, self.p0)
            epi_log = run_epi_percent(self.adj_lists, self.nodes, self.p0)
            make_graphs(edg_list, epi_log)
    ...
    def gen_ER(self):
        if self.txt_check():
            ...
            self.edges = int(self.edge_txt.get("1.0", 'end-1c'))
            self.p0 = int(self.p0_txt.get("1.0", 'end-1c'))
            self.adj_lists, nodes = erdos_renyi(
                "edgelists.txt", self.nodes, 0.05)
            edg_list = get_edge_list("edgelists.txt")
            epi_log = run_epi(self.adj_lists, self.nodes, self.p0)
            make_graphs(edg_list, epi_log)
            self.img_count = 0
    ...
    def gen_WS(self):
        if self.txt_check():
            ...
            self.edges = int(self.edge_txt.get("1.0", 'end-1c'))
            self.p0 = int(self.p0_txt.get("1.0", 'end-1c'))
            self.adj_lists, nodes = watts_stogatz(
                "edgelists.txt", self.nodes, 4, 0.2)
            edg_list = get_edge_list("edgelists.txt")
            epi_log = run_epi(self.adj_lists, self.nodes, self.p0)
            make_graphs(edg_list, epi_log)
            self.img_count = 0
    ...
    def txt_check(self):
        ...
        if nodes > edges:
            self.err_lbl.config(
                text="Nodes must be less than or equal to the number of edges.")
            self.root.update_idletasks()
            return False
        elif p0 >= nodes:
            self.err_lbl.config(
                text="Patient zero must be less than the number of nodes.")
            self.root.update_idletasks()
            return False
        return True
...
def run_epis(adj_lists, nodes, p0: int = 0):
    ...
    for _ in range(epidemics):
        ...
        # epi_log, temp, stop = run_epi_percent(adj_lists, nodes, p0)
        epi_log, temp, stop, reopen = run_epi_reopen(adj_lists, nodes, p0)
...
#     for _ in range(epidemics):
#         # epi_log = run_epi(adj_lists, nodes, p0)
#         epi_log = run_epi_percent(adj_lists, nodes, p0)
#         epi_log = [len(n) for n in epi_log]
#         epi_logs.append(epi_log)
#         lengths.append(len(epi_log))
#         pass
...
def run_epi(adj_lists: [], nodes: int, p0: int = 0):
    ...
    length = 0
    while num_infected > 0:
        if time_step == shutdown:
            for i in range(len(temp_list)):
                list_len = len(temp_list[i])
                indexes = []
                for j in range(list_len):
                    x = rand.random()
                    if x <= chance_remove:  # remove edges in both directions
                        indexes.append(j)
                        # remove the reciprical edge
                        # adj_lists[adj_lists[i][j]].remove(i)
                        # del adj_lists[i][j]
                for index in reversed(indexes):
                    del temp_list[i][index]
                    temp_list[temp_list[i][index]].remove(i)
...
def main():
    ...
    # epi_log, temp, tmp2 = run_epi_percent(adj_lists, nodes)
    epi_log, temp, tmp2, tmp3 = run_epi_reopen(adj_lists, nodes)
    make_graphs(edg_list, epi_log)
